...
let ethAdapter, safeSdkInstance, safeService;
...
const  useSafeSdk = () => {
  ...
  const approveTransfer = useCallback(
    async ({ safeTxHash, execTxn }: { safeTxHash: string; execTxn: Boolean }) => {
  ...
  const rejectTransfer = useCallback(
    async ({ safeTxHash, execTxn, nonce }: { safeTxHash: string | null; execTxn: Boolean; nonce: number }) => {
      if (!safeTxHash) {
        if (safeSdk && signer) {
  ...
  return { signer, safeSdk, safeService, safeAddress, isTxnExecutable, approveTransfer, rejectTransfer }
}
